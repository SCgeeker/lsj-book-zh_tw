...
使用jamovi 的基本分析模組`Frequencies`，就能完成以上的演算，以下使用資料集*Randomness.omv*示範。首先從點選“Analysis"主選單的點選“Frequencies”套件，接著選擇“One Sample Proportion Tests”的“$N$ Outcomes”，開啟分析工具設定視窗。接著選擇要分析的依變項（`choice_1`），移動到Variable對話框，再來勾選"Expected counts"，完成報表設定，如同 @fig-fig14-4 的畫面右方報表呈現的分析結果。接著會看到報表呈現與前述的手動計算相同的預期計數和統計資料，$\chi^2 =  8.44$，自由度為 $3$，$p=0.038$。更重要的是，我們不需要查表找臨界值判斷 p 值，因為 jamovi 輸出的p 值是來自$\chi^2$自由度是$3$的機率分佈。
...
我們再回到本章的開頭，再看一次撲克牌示範資料集。回憶一下細節，這個實驗的實驗設計是請參與者選擇兩次。因為有每位參與者第一次和第二次選擇的資料，可以拿來建立 @tbl-tab14-13 的列聯表，呈現第一次選擇與第二次選擇交叉列聯次數。
...
假如想知道第二次選擇是否取決於第一次選擇，我們能用獨立性檢定來分析，這裡要做的是看看這個表格的行和列之間是否存在某種關聯性。

另外，假如想知道平均而言，第二次選擇花色的次數是否與第一次的不同。這裡要做的是看看行總數是否不同於列總數。這就是麥內瑪檢定派上用場的時候。

以上問題的分析會產生不同的統計結果，結果總結在 @fig-fig14-9 。請注意因為檢定的目標不一樣，分析結果是不同的！

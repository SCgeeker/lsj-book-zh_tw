...
讀者在 @sec-Other-useful-distributions 初次認識卡方分佈，只有稍微了解什麼是**自由度**，這裡要再次解釋這項重要概念。請看一下 @fig-fig14-1 不同的卡方分佈，可知只要自由度改變了，那麼形狀劇烈改變。那麼**自由度**究竟是什麼呢？其實在解釋自由度與常態態分佈的關係時，就提到了一種解釋：自由度是可平方並可加成的*常態分佈變項數量*。當然，這樣的解釋相當抽象，對多數讀者沒有幫助。接著讓我們透過真實的資料，來嘗試理解自由度吧。
...
[^10-categorical-data-analysis-5]: 我必須強調，這是一種過度簡化的解釋。雖然這種解釋適用多數情況，但是偶爾會遇到自由度並非整數的狀況。為了不要讓讀者過於困擾，遇到這樣的狀況時，只要告訴自己“自由度”其實是一種有點混亂的數學概念，初學的讀者通常只會知道簡單的解釋而非完整的解釋。以入門課程來說，簡單的解釋通常是最好的學習材料，但讀者需要知道總有一天這樣的解釋無法幫助你更進一步的學習，如果沒有先做好心理準備，有天看到了 $df = 3.4$ 或類似的狀況就會感到困惑，以為你在這裡學到的是錯誤的觀念，其實是先不交待有些複雜的細節。
...
使用jamovi 的基本分析模組`Frequencies`，就能完成以上的演算程序，以下使用資料集*Randomness.omv*示範。首先從“Analysis"主選單的點選“Frequencies”套件，接著選擇“One Sample Proportion Tests”的“$N$ Outcomes”，開啟分析工具設定視窗。接著選擇要分析的依變項（`choice_1`），移動到Variable對話框，再來勾選"Expected counts"，完成報表設定，如同 @fig-fig14-4 的畫面右方報表呈現的分析結果。接著會看到報表呈現與前述的手動計算相同的預期計數和統計資料，$\chi^2 =  8.44$，自由度為 $3$，$p=0.038$。更重要的是，我們不需要查表找臨界值才能判斷 p 值，因為 jamovi 輸出的p 值是來自$\chi^2$自由度是$3$的機率分佈。
...
#| fig-cap: jamovi 的 $\chi^2$ 單樣本比例檢定(One Sample Proportion Tests)示範，報表表格顯示觀察到的次數和比例(Observed)以及期望的次數和比例(Expected)
...
也許讀者會遇到適合度檢定要檢驗的虛無假設涵蓋的所有類別，每一類別的機率並不是彼此相等的狀況，這樣應該要怎麼辦？例如，有套理論預測多數人心中會選擇紅色撲克牌的機率是$60\%$，選擇黑色撲克牌的機率是$40\%$，所以設定虛無假設的期望比例是紅心為 $30\%$，方塊為 $30\%$，黑桃為 $20\%$，梅花為 $20\%$。也就是說，理論預測紅心和方塊的期望次數是黑桃和梅花的 1.5 倍（$30\%$ : $20\%$ 等於 1.5 : 1 ）。儘管這套理論聽起來沒根沒據，還是能用jamovi檢定這套虛無假設。在設定視窗的左邊( @fig-fig14-4 )，點選"Expected Proportion"就能展開Ratio(預期比率)的選項。如此就能在待輸入的空格中，為變項所設定的類別輸入不同的。這個案例的變項是`choice_1`。jamovi能自動將各類別相對倍數，轉換為虛無假設的各類別比例數值，如同 @fig-fig14-5 的示範。
...
```{r}
#| label: fig-fig14-5
#| fig-cap: 根據虛無假設設定，更改單樣本比例檢定設定視窗的 $\chi^2$ 預期比率。
...
各類別的預期次數顯示於 @tbl-tab14-6 。

<!--- The expected counts are now shown in @tbl-tab14-6. --->

```{r}
#| label: tbl-tab14-6
#| tbl-cap: 另一種虛無假設設定的各花色預期次數
#Expected counts for a different null hypothesis
huxtabs[[10]][[6]]
```

這次計算的假設和預期次數，與假設各類別機率相等的結果不一樣了，因此， $\chi^2$ 檢定統計數和 p 值也有變化：$\chi^2$ 統計量為 4.74，自由度為 3，$p = 0.182$。新的p值是最令人沮喪的，和 @sec-The-p-value-of-a-test 的示範相比，這次分析結果無法拒絕虛無假設。最無奈的是，就算虛無假設是一種沒什麼道理的理論，現實的資料無法形成足夠的反駁證據。
...
知道了適合度檢定的執行程序，也要知道如何使用jamovi來完成報告的撰寫。畢竟，花心思設計和執行實驗，然後分析資料，不告訴別人的話是沒有意義的！所以接著要說明準備分析報告需要注意的事情。繼續以撲克牌花色的案例說明，如果想將這些結果寫成一篇論文，符合規範的報告寫作示範如同下一段：

> 這項實驗的200位參與者有64人的首選是紅心，51人選擇方塊，50人選擇黑桃，35人選擇梅花。以卡方適合度檢定，分析四種花色首次被選擇機率是否相同，顯著結果表示人們有意識選擇花色時並非完全隨機（$\chi^2(3) = 8.44, p< .05$）。

報告的敘述很直接，讀起來中規中矩。儘管如此，讀者應該留意報告中的幾個的重點：

- *描述統計要擺在統計檢定之前*。在呈現檢定之前，讀者需要知道目標資料的重要訊息，呈現描述統計是一種很好的寫作方法。寫報告時一定要記得，讀者對於資料的了解遠不如寫作者。所以，除非先向讀者妥善地描述資料特徵，否則統計檢定對讀者來說毫無意義，只會覺得作者眼高於頂。

- *告訴讀者這次分析要測試的虛無假設*。老實說，多數作者並不會這樣做，但是當文字表達存在歧義，或者讀者不見得熟悉作者使用的統計方法時，反而是一種好的寫作策略。很多時候讀者可能不知道或不記得作者使用的檢定方法細節，所以給一些提示是一種體貼！拿適合度檢定來說，寫作者通常假定受過科學訓練的讀者大都了解執行細節，因為許多入門統計課程都會介紹這套方法。不過，簡要明確地陳述虛無假設仍然是好策略，因為虛無假設的設定會因為檢驗的目標而有所不同。就像撲克牌範例的第一種虛無假設是四種花色的機率相同（即，$P1 = P2 = P3 = P4 = 0.25$），雖然這套假設並沒有什麼特別的。另一方面，適合度檢定也可以分析另一種虛無假設：$P_1 = 0.7$和$P2 = P3 = P4 = 0.1$。所以，向讀者解釋虛無假設還是有幫助的。另外，注意以上報告示範是用文字而不是數學符號描述虛無假設，這是完全可以接受的。當然寫作者也可以用數學符號描述，但是考慮大多數讀者閱讀文字比符號更容易，所以大多數寫作者傾向儘可能用文字描述虛無假設。

- *構築"統計區塊"*。報告檢定結果的那句話，不僅提到結果顯著，還有用括號集合數學符號的“統計區塊”，其中包括所有“關鍵”統計訊息。卡方適合度檢定要報告的訊息包括檢定統計量（8.44）、用於檢定的分佈信息：自由度為3的$\chi^2$分佈，通常簡寫為$\chi^2 (3)$，以及結果是否顯著的p值標記（$p< .05$）。每個檢定報告的統計區塊要呈現的特定訊息各不相同，因此後介紹的每一種檢定方法，都會示範統計區塊應該如何組織。[^10-categorical-data-analysis-7] 總之，一般原則是寫作者應該提供足夠的訊息，讓讀者認為有必要時，能自行檢核檢定結果。

- *解釋分析結果*。除了指出結果有沒有顯著之外，示範還有結果的解釋。這也是一種友善讀者的寫作方式，告訴他們從資料中發現了什麼事情。如果沒有給出解釋，讀者很難理解其中有什麼發現。[^10-categorical-data-analysis-8]

[^10-categorical-data-analysis-7]: 如何報告統計結果的規範因學科性質而異。因為這本書以心理學系學生為主要讀者，會採用心理學的寫作規範。但是，向讀者提供足夠的訊息，讓讀者能夠檢核結果，是不分學科的普遍原則。

[^10-categorical-data-analysis-8]: 對某些人來說，這樣的建議可能聽起來有點奇怪，或者至少與研究報告的編寫建議有些衝突。許多學生通常會被告知報告的“結果”部分，是用來描述資料並報告統計分析，“討論”部分才是提供解釋。某種程度上這樣的建議並沒有錯，但是教學者經常會只照字面的意思教授。專業寫作者的通常做法是在結果部分提供快速簡單的解釋說明，以便讀者理解這份資料告訴我們什麼。然後，討論部分再從更大的視野，講述這次研究的結果，如何調整現存科學文獻所構築的知識體系。簡單來說，不要讓“解釋屬於討論”這個建議，增加結果部分的閱讀難度，更重要是讓讀者明白作者要表達的意思。

如同各式各樣出版品，寫作者應該關注的是幫助讀者明白事物原理。寫作的首要目的是與他人溝通，撰寫結果報告的目的也是如此。身為教師的人通常會看過無數份報告、論文甚至科學文章的結果充斥胡言亂語，因為多數寫作者只關注呈現數字，卻忘記與人類讀者的交流責任。

> *統計數字和宗教經文都有隱藏撒旦的微笑*[^10-categorical-data-analysis-9]
> -- H.G. 威爾斯

[^10-categorical-data-analysis-9]: 如果讀者讀得很仔細，也是對數學表達要求嚴格的人，那麼這一節卡方檢定結果的寫作可能會讓你們有點困擾。讀者可能會覺得 "$\chi^2(3) = 8.44$"這樣的寫法似乎有點不對勁。畢竟是說適合度檢定統計數等於8.44，所以是不是應該寫成 $\chi^2 = 8.44$ 或者 $GOF = 8.44$ 比較好？有的讀者可能認為這會讓人認為是錯字，因為$\chi$和X長得非常像，有人會混淆表示取樣分佈（$\chi^2$與df=3）與檢定統計量（$\chi^2$）的符號。奇怪的是，多數專業讀者不這應認為。$\chi^2$(3)= 8.44 其實是一種極度縮略的寫法，意思是“檢定統計數的取樣分佈為$\chi^2$(3)，檢定統計數的值為8.44”。從某種意義來看，這種寫法有點愚蠢，許多統計檢定的統計值都會適配卡方取樣分佈。儘管適合度檢定很常見，最後得到的$\chi^2$統計數只是眾多統計數的一種。在一個講究規則，依賴組織運作的世界，總會有為檢驗統計定和取樣分佈各取一個專有名詞的呼聲。有了專有名詞，報告裡的統計區塊就可以告訴讀者究竟算了什麼。雖然有部分統計數是可行的，像是Pearson適合度檢定使用的檢定統計值寫作$\chi^2$，但是有一種與G-test$^a$ [@Sokal1994]關係密切的檢定稱為G-test，其中檢定統計數寫作$G$。事實上，Pearson適合度檢定和G-test都能檢定相同的虛無假設，取樣分佈也是自由為$k-1$的卡方分佈。如果我對撲克牌資料進行G-test而不是適合度檢定，得到一個檢定統計數就是$G = 8.65$，比起之前算出的$\chi^2 = 8.44$稍微大一點，並且p值稍微小一點$p = .034$。假設報告慣例是先寫檢定統計數，然後是取樣分佈，然後是p值。如此一來兩種檢定會寫成完全不同的統計區塊：卡方檢定的結果是$\chi^2 = 8.44$，$\chi^2(3)$，$p = .038$，而G-test的版本是$G = 8.65$，$\chi^2(3)$，$p = .034$。然而，根據縮略報告標準，卡方檢定的結果寫作$\chi^2(3) = 8.44, p =.038$，G-test的結果寫作$\chi^2(3) = 8.65, p = .034$，所以會搞不清楚研究者是執行了那種檢定方法。<br>那麼為何無法讓統計區塊清楚表示是執行那種檢定方法嗎？根本原因是宇宙的本質就是混沌。身為統計工具的我們當然希望有整齊有序的寫作規範，讓統計報告像工廠產品一樣被設計，但這不是宇宙的運作方式。統計是一門與其他學科一樣集合眾人智慧的領域，持續由許多學者以分散、協作又競爭的模式更新知識，所以沒有人能真正完全掌握統計的寫作規則。你我正在用的資料分析工具並不是來自一位統計之神的創造，而是很多學者的發明累積作。要經過發表在學術期刊，經過其他學者實做、糾正和修改，然後其他學者寫到教科書，才能向學生解釋。因此，有很多檢定統計數甚至沒有名字，只是被賦予與對應的取樣分佈相同的名字。稍後將看到許多這類例子，任何遵循$\chi^2$分布的檢定統計數，通常被稱為“卡方統計數”，遵循$t$分布的任何統計量被稱為“t統計數”。但是，如同$\chi^2$與$G$，即使統計數來自相同的取樣分佈，兩者仍然是不一樣的東西。所以有時候寫清楚實際執行的檢定方法是很好的主意，特別是使用一些不常見的統計方法。如果報告只寫“卡方檢定”，讀者會不清楚這是在談那種檢定。儘管如此，由於最常見的兩種卡方檢定是適合度檢定和獨立性檢定，大多數有學過統計的讀者可能都會猜到。儘管如此，這仍然是需要注意的事情。—$^a$ 讓統計報告寫作變得複雜的是，G-test是一種被稱為似然比檢定特別方法。由於這本書中沒有介紹LRT，無法深入介紹，但這是值得讀者好好了解的統計方法。
...
<!--- 現在我們已經了解了如何從虛無假設中計算期望頻率，接下來的步驟與適合度檢定相似。首先，對於每個 $O_{ij}$ 和 $E_{ij}$，我們計算 $(O_{ij} - E_{ij})^2 / E_{ij}$ 的值。然後，我們將這些值相加，得到一個名為卡方的統計量（$\chi^2$）。這個統計量也遵循卡方分布，因此我們可以將其用於計算 p 值，並進行檢驗。實際上，與適合度檢定相似，卡方獨立性檢驗的自由度是由資料中的類別數量決定的，而不是預先知道的。
...
好吧，既然我們知道了檢驗是如何進行的，讓我們看看如何在 jamovi 中完成它。雖然讓您長時間地經歷繁瑣的計算以便被迫學習可能很有吸引力，但我認為這是沒有意義的。在上一節中，我已經向您展示了如何針對適合度檢定進行長時間的操作，而且由於獨立性檢驗在概念上沒有任何不同，所以您不會通過長時間的操作學到任何新的東西。因此，我將直接向您展示簡單的方法。在 jamovi 中運行檢驗（“頻率” - “列聯表” - “獨立樣本”）之後，您只需查看 jamovi 結果窗口中列聯表下方，那裡就是 $\chi^2$ 統計量。這顯示了一個 $\chi^2$ 統計值為 10.72，2 d.f.，p-value = 0.005。
...
對於大部分讀者來說，搞清楚這個函數代表的機率分佈是長什麼樣子，是有些困難的。用專業術語來表示的話，這個機率函數符合所謂的超幾何分佈(hypergeometric distribution)。要計算 p 值，首先要計算 @tbl-tab14-10 的數值，或者計算這份表格內的數值有多“極端”的機率。[^10-categorical-data-analysis-17]在 20 世紀 20 年代，即使是狀況最單純的研究問題，要處理這樣的計算是相當令人害怕的工作。還好到了今日，只要表格不是太大，樣本數不是太龐大，一般計算設備都能輕鬆處理。真正棘手的問題是，要弄清楚一個列聯表比另一個列聯表更“極端”，要怎麼呈現出來？最簡單的方法是，出現機率最低的表格是最極端的，我們只要知道這個表格的 p 值。

[^10-categorical-data-analysis-17]: 不必懷疑，費雪精確檢定是根據費雪一派對 p 值的解釋，不是尼曼派的！請參考 @sec-The-p-value-of-a-test 。
...
要使用jamovi執行費雪精確檢定，只要從`Analysis`選單選擇`Frequencies`-`Independent Samples`開啟`Contingency Tables`設定視窗，其中`Statistics`的子選項有`Fisher's excat test`。已經載入`Salem`資料集的話，只要設定好變項，就能在報表看到費雪精確檢定的統計結果。因為這種檢定方法只有輸出p值，由於這個範例的p 值足夠小（p = .036），足以拒絕那些正在被火燒的c人，和還沒有被火燒的犯人一樣快樂的虛無假設。輸出結果如同 @fig-fig14-7。
...
了解麥內瑪檢定的計算原理，接著了解如何用jamovi實作。以上解說用的原始資料，都存在*agpp.csv*資料集，其中包含三個變項：`id`變項標記每位參與者，`response_before`記錄每位參與者第一次被問到問題的回答，另一個`response_after`記錄每位參與者第二次被問到同樣問題的回答。留意一下每位參與者在這項調查是被重覆測量的。
為了執行檢定，從jamovi模組選單選擇 'Analyses' - 'Frequencies' - 'Contingency Tables' - 'Paired Samples'，將`response_before`放入 'Rows' 對話框，`response_after`放入 'Columns' 對話框。結果視窗會出現如同 @fig-fig14-8 的列聯表，以及麥內瑪檢定統計結果。
...
#| label: fig-fig14-8
#| classes: .enlarge-image
#| fig-cap: jamovi中的麥內瑪檢定輸出結果
#McNemar test output in jamovi
knitr::include_graphics("images/fig10-8.png")
```

我們剛剛完成了一個麥內瑪檢定，可以確定受訪者在看過廣告後，會不會如同未看廣告前一樣支持AGPP。檢定結果是顯著的（$\chi^2(1)= 12.03, p< .001$），表示受訪者的態度並非如調查者的預期。事實上，廣告宣傳似乎 產生了負面影響：看過廣告後，人們投票支持AGPP的意願可能更低。考慮一下典型政治宣傳廣告的製作水準，這種結果是很合理的。
...
### 與獨立性檢定的區別 {#sec-difference-McNemar-independence}

最後，讓我們回顧本章一開始的撲克牌示範資料集。這個實驗的實驗設計是請參與者選擇兩次。因為有每位參與者第一次和第二次選擇的資料，可以拿來建立如同 @tbl-tab14-13 的列聯表，呈現第一次選擇與第二次選擇交叉列聯次數。
...
```{r}
#| label: tbl-tab14-13
#| tbl-cap: Randomness資料集交叉列聯第一次與第二次選擇次數
#Cross-tabulating first against second choice with the *Randomness.omv* (cards) data
huxtabs[[10]][[13]]
```

假如我們懷疑第二次選擇是否取決於第一次選擇，就能用獨立性檢定來分析，這裡的檢定目標是確認這份表格的行與列之間，是否存在某種關聯性。

另外，假如想知道以平均來看，第二次選擇花色的次數是否與第一次的選擇不同？這裡的檢定目標是確認行的總次數是否不同於列的總次數。這裡就是麥內瑪檢定派上用場的時候。

以上兩種問題的分析會產生不一樣的統計結果，兩種檢定結果匯整在 @fig-fig14-9 。請注意因為檢定的目標不一樣，分析結果是不同的！

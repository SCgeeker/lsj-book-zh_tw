...
使用jamovi 的基本分析模組`Frequencies`，就能完成以上的演算，以下示範使用資料集的*Randomness.omv*。

jamovi 的預設分析套件`Frequencies`提供完成以上計算的工具，以下使用 *Randomness.omv* 示範。首先從主選單“Analysis"列出的套件按鈕，點選“Frequencies” - “One Sample Proportion Tests”- “$N$ Outcomes”。在分析工具設定視窗選擇要分析的變項（`choice_1`），到Variable對話框。接著勾選"Expected counts"，完成報表設定，如同 @fig-fig14-4 的畫面右方報表呈現的分析結果。讀者可以看到報表呈現與前述的手動計算相同的預期計數和統計資料，$\chi^2 =  8.44$，自由度為 $3$，$p=0.038$。更重要的是，我們不需要查表找臨界值判斷 p 值，因為 jamovi 也輸出$\chi^2$的自由度是$3$所對應的 p 值。
...
#| fig-cap: jamovi 中的 $\chi^2$ 單樣本比例檢定(One Sample Proportion Tests)，報表表格顯示觀察到的頻率和比例(Observed)以及期望的頻率和比例(Expected)
...
也許讀者會遇到適合度檢定要檢驗的虛無假設涵蓋的所有類別，並不是機率都相等，應該怎麼辦？例如，假設有套理論預測多數人選擇花色是紅色的撲克牌機率是$60\%$，選擇花色是黑色的撲克牌機率是$40\%$，所以設定虛無假設的期望比例是紅心為 $30\%$，方塊為 $30\%$，黑桃為 $20\%$，梅花為 $20\%$。也就是說，紅心和方塊的期望次數是黑桃和梅花的 1.5 倍（$30\%$ : $20\%$ 等同於 1.5 : 1 ）。儘管這套理論聽起來有些無厘頭，還是能用jamovi檢定這套虛無假設。從設定視窗左方( @fig-fig14-4 )，點選"Expected Proportion"就能展開Ratio(預期比率)的選項。如此就能在待輸入的空格中，為變項所設定的類別輸入不同的比例值。這個案例的變項是`choice_1`。jamovi能自動將比率轉換為虛無假設的比率數值，如同 @fig-fig14-5 。
...
```{r}
#| label: fig-fig14-5
#| fig-cap: 在單樣本比例檢定設定視窗，填寫 $\\chi^2$ 的預期比率。
...
重新計算的假設和預期次數與前一次的結果不一樣了，因此，我們的 $\chi^2$ 檢定統計數和 p 值也有所不同，$\chi^2$ 統計量為 4.74，自由度為 3，$p = 0.182$。更新的p值是最令人不高興的，這麼一來就不能拒絕虛無假設（請讀者回顧 @sec-The-p-value-of-a-test ）。最無奈的是，就算虛無假設代表一種沒什麼道理的理論，現在的資料並無法提供足夠的證據來反駁。
...
現在知道了適合度檢定的執行方法，也知道如何使用jamovi來完成。接下來讀者需要知道的是如何撰寫結果。畢竟，花心思設計和執行實驗，然後分析資料，如果不告訴別人結果是沒有意義的！所以接著來談談準備分析報告需要注意的事情。我們繼續以撲克牌花色的案例說明。如果想將這些結果寫成一篇論文，那麼符合慣例的報告如同下一段：

> 這項實驗200位參與者有64人首選紅心，51人選擇方塊，50人選擇黑桃，35人選擇梅花。以卡方適合度檢定，分析四種花色被選擇機率是否相同，顯著表明人們在選擇花色時並非完全隨機（$\chi^2(3) = 8.44, p< .05$）。

執告敘述很直接，讀起來中規中矩。儘管如此，讀者應該留意報告中的幾個的重點：

- *描述統計要擺在統計檢定之前*。在呈現檢定之前，讀者需要知道目標資料的重要信息，這通常是一個很好的寫作方法。寫報告時一定要記得，讀者對於資料的了解遠不如寫作者。所以，除非向讀者妥善地描述，否則統計檢定對讀者來說毫無意義，只會覺得作者看不起人。

- *告訴讀者這裡要測試的虛無假設*。老實說，多數作者並不會這樣做，但是當文字表達存在歧義，或者讀者不見得熟悉這裡使用的統計工具時，通常是一個好的寫作策略。很多時候讀者可能不知道或不記得報告使用的檢定方法細節，所以給一些提示是一種禮貌！以適合度檢定來說，寫作者通常可以假定受過科學教育的讀者大都了解執行細節（因為大多數入門統計課程都會介紹）。然而，簡要且明確地陳述虛無假設仍然是一個好策略，因為虛無假設可能因為檢驗目的而有所不同。就像撲克牌範例的第一種虛無假設是四個花色的機率相同（即，$P1 = P2 = P3 = P4 = 0.25$），但是這個假設並沒有什麼特別的。另一方面，我們也可以使用適合度檢定分析另一種虛無假設：$P_1 = 0.7$和$P2 = P3 = P4 = 0.1$。所以，向讀者解釋虛無假設還是有幫助的。另外，注意以上報告示範是用文字而不是數學符號描述虛無假設，這是完全可以接受的。當然也可以用數學符號描述，但是考慮大多數讀者閱讀文字比符號更容易，所以大多數作者傾向儘可能用文字描述虛無假設。

- *構築"統計區塊"*。在報告檢定結果的那句話裡，不僅僅說結果顯著，還加入了用括號集合數學符號的“統計區塊”，其中包括所有“關鍵”統計訊息。卡方適合度檢定要報告的訊息包括檢定統計量（8.44）、用於檢定的分布信息（具有3個自由度的$\chi^2$分佈，通常簡寫為$\chi^2$(3)），以及結果是否顯著的標記（$p< .05$）。每個檢定報告的統計區塊要呈現的特定訊息各不相同，因此後續介紹一個檢定方法時，都會向讀者展示統計區塊應該如何組織。[^10-categorical-data-analysis-7] 不過，一般原則是寫作者應該提供足夠的訊息，讓讀者在有必要時檢核檢定結果。

- *解釋分析結果*。除了指出結果有沒有顯著之外，示範還提供結果的解釋。這對也是一種友善讀者的寫作方式，告訴他們從資料中發現了什麼事情。如果沒有這樣的部分，讀者很難理解有什麼發現。[^10-categorical-data-analysis-8]

[^10-categorical-data-analysis-7]: 如何報告統計結果的慣例因學科性質而異。因為這本書以心理學系學生為主要讀者，會採用心理學的寫作慣例。但是，向讀者提供足夠的訊息，讓他們可以檢核結果，是普遍的的一般原則。

[^10-categorical-data-analysis-8]: 對某些人來說，這個建議可能聽起來有點奇怪，或者至少與編寫技術報告的常見建議互相衝突。許多學生們通常會被告知報告的“結果”部分，是用來描述資料並報告統計分析，“討論”部分才是提供解釋。在某種程度上這樣的建議並沒有錯，但是教學者經常會只照字面的意思教授。原作者的通常做法是在結果部分提供快速且簡單的解釋，以便讀者理解這份資料告訴我們什麼。然後，在討論部分再從更大的視野，講述這次研究的結果，如何調整現存科學文獻所構築的知識體系。簡單來說，不要讓“解釋屬於討論”這個建議，增加結果部分的閱讀難度，更重要是讓讀者明白作者要表達的意思。

如同各式各樣出版品，寫作者應該關注的是為讀者解釋事物。寫作的第一目的是與另一個人溝通，同樣是撰寫結果報告的目的。身為教師的人通常會看過無數份報告、論文甚至科學文章的結果部分都是胡言亂語，因為作者只關注呈現數字，卻忘記與人類讀者的交流責任。

...
現在我要正式介紹 **相關係數(correlation coefficient)** ，來更明確地說明上述的概念（更具體地說，是介紹皮爾森相關係數），慣例書寫符號是 $r$。在下一節，我將更精確地定義兩個變項 $X$ 和 $Y$ 之間的相關係數 $r_{XY}$，值域涵蓋-1到1。當$r = -1$時，表示我們有一個完全的負相關；當$r = 1$時，表示我們有一個完全的正相關；當$r = 0$時，表示完全沒有關係。 @fig-fig10-4 展示幾種不同相關性的散佈圖。

[其他技術細節 [^correlation-and-linear-regression-3]]

[^correlation-and-linear-regression-3]: Pearson相關係數的公式可以用幾種不同的方式來表示。最簡單的方式是將公式分為兩個步驟。首先，讓我們介紹**共變異數（covariance）**的概念。兩個變量$X$和$Y$的協方差是變異數概念的一般化，是一種數學上簡單描述兩個變量之間關係的方式，但對人類來說並不太有信息量: $$Cov(X,Y)=\frac{1}{N-1}\sum_{i=1}^N(X_i-\bar{X})(Y_i-\bar{Y})$$ 由於我們要乘以一個與$X$有關的量和一個與$Y$有關的量，然後再取平均值，因此你可以將協方差的公式視為$X$和$Y$之間的“平均交叉乘積”。協方差具有很好的性質，如果$X$和$Y$之間完全沒有關係，那麼協方差就是零。如果它們之間的關係是正向的（如@fig-fig10-4所示），則協方差也是正的，如果關係是負的，則協方差也是負的。換句話說，協方差捕捉到了相關性的基本定性概念。不幸的是，協方差的原始大小並不容易解釋，因為它取決於$X$和$Y$所表達的單位，而且更糟糕的是，協方差本身所表達的單位非常奇怪。例如，如果$X$是dani.sleep變量（單位：小時）而$Y$是dani.grump變量（單位：壞脾氣），那麼協方差的單位是$hours \times grumps$。我甚至不知道那是什麼意思。Pearson相關係數r解決了這個解釋問題，方法與z分數標準化原始分數的方式非常相似，通過除以標準差進行標準化。但是，由於我們有兩個變量對協方差作出貢獻，所以只有當我們將標準差除以兩個變量。由於有兩個變數影響到共變異數，因此標準化只能在除以兩個變數的標準差時有效。換句話說，兩個變數$X$和$Y$的相關係數可以寫成以下的公式：$$r_{XY}=\frac{Cov(X,Y)}{\hat{\sigma}_X\hat{\sigma}_Y}$$<br />---<br />$^a$ 和變異數、標準差一樣，實際上我們在計算時除以的是 $N-1$ 而不是 $N$。<br /> $^b$ 這是一個過於簡化的說法，但對於我們的目的而言已足夠。
...
通過標準化協方差，我們不僅保留了前面討論過的協方差的所有優點，而且r的實際值是有意義的: r = 1意味著完美的正相關，$r = -1$意味著完美的負相關。稍後我會在[解讀相關係數](10-Correlation-and-linear-regression.html#sec-interpreting-a-correlation)一節中更詳細地談論這一點。但在此之前，讓我們看一下如何在jamovi中計算相關性。
...
只要在jamovi'迴歸'模組選單，選點要計算的相關係數，就能計算所有納入變項對話框裡的變項之間相關係數，如同 @fig-fig10-5 的示範，未出錯的話，報表會輸出'相關矩陣'。
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: 這個圖解展示虛擬資料集的兩個變項"工作時間"和"得分"之間的關係，這個資料集只有10位學生（每個點代表一個學生）。圖中的直線顯示兩個變項之間的線性關係，兩者之間有很強的皮爾森相關係數$r = .91$。不過有趣的是，兩個變項之間存在一個完美的單調函數關係。這條直線顯示，根據這份虛擬資料，增加工作時間總是會增加得分，這反映在斯皮爾曼等級相關係數$\rho = 1$。然而，由於這個資料集很小，因此仍然存在一個問題：那一種係數是真正描述兩個變項的關係。
...
data.frame(
`學生編號` = paste("學生",1:10),
`工作時間次序` = c(1,10,6,2,3,5,4,8,7,9),
`得次序` = c(1,10,6,2,3,5,4,8,7,9)
) %>%
  kbl(booktabs=TRUE,escape = F, align = "c") %>%
  kable_paper(full_width = T)
```

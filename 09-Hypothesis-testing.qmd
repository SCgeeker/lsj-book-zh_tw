...
[^09-hypothesis-testing-2]: 技術性註記。這一章的敘述與許多入門教材的常見說明有些微不同。經典的虛無假設檢定理論是20世紀初期，由Ronald Fisher和Jerzy Neyman發展出來的，但是Fisher和Neyman對於理論的運作有非常不同的看法。現今大多數教材所採用的檢定理論是兩種觀點的混合。這裡所採用的解說較接近Neyman的風格，特別在解釋*p*值意義的部分。
...
我們從一個原作者虛構的狂想開始談吧：*我認為最終，人類都會屈服於瘋狂。我真正想做的研究，將在我升任正教授的時候開始。那一天到來時，我在象牙塔中會受到終身職的保障，我總算能夠拋棄理智，完全投入那個最沒有成效的心理研究領域：證實人類有超感官知覺（ESP）。*[^09-hypothesis-testing-3]

[^09-hypothesis-testing-3]: 在此對真心相信超感官知覺（ESP）存在的朋友說聲抱歉，根據我（原作者）所讀過的ESP相關文獻，我認為這種研究是不切實際的。不過認真地說，其中一些研究的設計是嚴謹，所以從心理學研究設計的角度來看，這是一個有趣的領域。當然，身在一個自由國家，只要你願意，你可以花費時間和精力來證實我錯了，但我認為這真的不是一個發揮個人智慧的實在途徑。
...
在這一點上，你們中有些人可能會想知道這是一個“真實”的假設檢定，還是只是我編造的一個玩具例子。這是真實的。在之前的討論中，我從第一原理構建了這個測試，認為這是你在現實生活中可能遇到的最簡單的問題。然而，這個測試已經存在。它被稱為二項式檢定，並且在 jamovi 中作為統計分析之一實現，當你點擊“頻率”按鈕時，就可以使用。要測試的虛無假設是回答機率為一半，即 $p = 0.5$，[^09-hypothesis-testing-9] 使用二項式測試中 $x=62$，$n=100$ 的數據，可以在 binomialtest.omv 數據文件中找到，獲得的結果如圖9-4所示。

[^09-hypothesis-testing-9]: 注意，此處的 p 與*p*值無關。在 jamovi 中進行二項檢定時，$p$ 參數代表根據虛無假設猜測正確答案的機率，也就是 $\theta$ 值。換句話說，$p$ 是指虛無假設下答對的機率。
...
這個輸出結果對你來說可能還很陌生，但你可以看到它大致上告訴你正確的事情。具體而言，$0.02$ 的*p*值小於通常選擇的 $\alpha = 0.05$，因此你可以拒絕虛無假設。隨著我們的學習進展，我們將會更深入地探討如何閱讀這類輸出結果，希望過一段時間後，你會發現它變得相當容易閱讀和理解。
...
在前面的章節中，我們強調了統計假設檢定背後的主要設計原則，那就是試圖控制型一錯誤率。當我們設定 $\alpha = .05$ 時，我們嘗試確保只有 $5%$ 的真假零假設會被錯誤拒絕。然而，這並不意味著我們不關心型二錯誤。實際上，從研究者的角度來看，當零假設實際上為假卻未能拒絕時的錯誤非常令人困擾。考慮到這一點，假設檢定的次要目標是嘗試最小化型二錯誤率 $\beta$，儘管我們通常不會用「最小化型二錯誤」來形容它。相反，我們會用「最大化考驗力」來描述。由於考驗力被定義為 $1-\beta$，這兩者是相同的。
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: 當總體參數值為 $\theta=0.55$，對立假設為成立時的抽樣分佈情況。圖中垂直的藍色線代表總體參數 $\theta$ 的值，而曲線則表示各種可能的樣本平均值的分佈情況。圖中藍色區域代表拒絕虛無假設的棄卻域，而這個棄卻域在這個例子中是 $\alpha=0.05$，即拒絕虛無假設的機率為 $5%$。可以看出，如果樣本平均值落在藍色區域，就可以拒絕虛無假設。
...
讓我們花一點時間來思考什麼是型二錯誤。當對立假設成立時，但我們仍然無法拒絕虛無假設時，就發生了型二錯誤。理想情況下，我們可以計算一個單一的數字 $\beta$，以告訴我們型二錯誤率，就像我們可以將 $\alpha = .05$ 設定為型一錯誤率一樣。不幸的是，這要做起來比較棘手。注意到，在我的 ESP 研究中，對立假設實際上對應著許多可能的 $\theta$ 值。事實上，對立假設對應的是除了 0.5 之外的每個 $\theta$ 值。讓我們假設某人選擇正確答案的真實機率為 55%（即 $\theta = .55$）。如果是這樣，那麼 $X$ 的真實取樣分佈不是虛無假設所預測的分佈，因為 $X$ 的最可能值現在是 $100$ 次中有 $55$ 次。不僅如此，整個取樣分佈現在也發生了偏移，如圖@fig-fig9-5所示。當然，關鍵區域並不會改變。根據定義，棄卻域是基於虛無假設所預測的。在這個圖中，我們看到的是，當虛無假設錯誤時，更大比例的取樣分佈落在棄卻域中。當然，這正是應該發生的事情。當虛無假設錯誤時，拒絕虛無假設的機率更大！然而，$\theta = .55$ 不是唯一符合對立假設的可能值。現在讓我們假設 $\theta$ 的真實值實際上是 $0.7$。當發生這種情況時，取樣分佈會發生什麼變化？答案如圖 @fig-fig9-6 所示，當 $\theta = 0.7$ 時，幾乎整個取樣分佈都移動到了棄卻域中。因此，如果 $\theta = 0.7$，我們正確拒絕虛無假設（即檢定的能力）的機率比 $\theta = 0.55$ 要大得多。簡而言之，雖然 $\theta = .55$ 和 $\theta = .70$ 都是對立假設的一部分，但型二錯誤率是不同的。
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: 在對立假設成立的情況下，當母體參數值為 $\theta = 0.70$ 時的取樣分佈。圖中顯示，幾乎整個分佈都在拒絕域中，也就是在這種情況下，拒絕虛無假設的機會相對於 $\theta = 0.55$ 的情況要大得多。
...
這就意味著檢定的考驗力（即$1-\beta$）取決於 $\theta$ 的真實值。為了說明這一點，我已經計算出了在所有 $\theta$ 值下拒絕虛無假設的預期概率，並在 @fig-fig9-7 中繪製了這些數值。這張圖描述了通常稱為檢定的考驗力函數。它是一個很好的評估檢定的好壞，因為它實際上告訴您對所有可能的 $\theta$ 值的考驗力（$1-\beta$）。正如您所看到的，當 $\theta$ 的真實值非常接近 $0.5$ 時，檢定的考驗力會急劇下降，但當它距離 $0.5$ 越遠時，考驗力就越大。
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: 當真正的$\theta$值不同於虛無假設所指定的值（即$\theta=0.5$）時，我們拒絕虛無假設的機率，也就是檢定的考驗力。可以明顯看出，當$\theta$與$\theta=0.5$非常不同時，測試具有更高的考驗力（即更高的正確拒絕虛無假設的機率）。需要注意的是，當$\theta$確實等於$0.5$時（以黑點表示），虛無假設實際上是成立的，此時拒絕虛無假設是一種型一錯誤。
...
@fig-fig9-7 展示了一個關於假設檢定的基本點。如果真正的世界狀態和虛無假設預測的狀態非常不同，那麼考驗力將非常高；但是如果真正的世界狀態與虛無假設相似（但不相同），那麼檢定的考驗力將非常低。因此，有一些方法可以量化真正的世界狀態與虛無假設之間的“相似程度”，這對於估計考驗力非常有用。一個可以做到這點的統計量被稱為效果量（例如 @Cohen1988; @Ellis2010）。效果量在不同的上下文中定義可能略有不同（因此，本節僅以一般性討論），但它所嘗試捕捉的質性想法始終相同（例如，參見 @tbl-tab9-6）。真實的母群參數與零假設假定的參數值之間有多大的差異？在我們的ESP例子中，如果讓$\theta_0 = 0.5$表示虛無假設假定的值，$\theta$表示真實值，那麼一個簡單的效果量可以是真實值和虛無假設值之間的差異（即$\theta - \theta_0$），或者可能只是這個差異的大小，$abs(\theta - \theta_0)$。
...
```{r}
#| label: tbl-tab9-6
#| tbl-cap: 這段說明是關於統計顯著性和效應大小之間關係的一個簡單指南。基本上，如果你沒有得到顯著的結果，那麼效應大小就不太有意義，因為你沒有任何證據表明它是真實存在的。另一方面，如果你確實有一個顯著的效應，但是你的效應大小很小，那麼你的結果（雖然是真實的）可能並不是很有趣。但是，這個指南非常粗略。它很大程度上取決於你研究的具體內容。在某些情況下，即使效應很小，它仍可能具有重大的實際意義。因此，不要太過認真地看待這個表格，它只是一個粗略的指南而已。
...
為什麼要計算效果量？假設你已經進行了實驗，收集了數據，在進行了假設檢定後得到了一個顯著效果。那麼只說你得到了一個顯著效果，這樣就足夠了嗎？當然不完全是。是的，進行假設檢定的目的是試圖證明虛無假設是錯誤的，但這並不是我們唯一關心的事情。如果虛無假設聲稱$\theta=0.5$，並且我們證明了它是錯誤的，我們只是講述了一半的故事。拒絕虛無假設意味著我們相信$\theta \neq 0.5$，但是$\theta=0.51$和$\theta=0.8$之間存在很大的差異。如果我們發現$\theta=0.8$，那麼不僅僅是虛無假設錯了，它似乎錯得非常厲害。另一方面，假設我們成功地拒絕了虛無假設，但真實值$\theta$看起來只有0.51（這只有在非常大的研究中才可能發生）。當然，虛無假設是錯的，但我們並不確定這是否重要，因為效果量非常小。在我的ESP研究中，我們可能仍然在乎，因為任何對真正的超能力的證明都是相當酷的，但在其他情況下，即使有$1%$的差異[^09-hypothesis-testing-10]，它通常也不是很有趣，即使它是真正的差異。例如，假設我們正在研究男女之間高中考試成績的差異，結果女性的成績平均比男性高$1%$。如果我有來自成千上萬名學生的數據，那麼這種差異幾乎肯定是統計上顯著的，但無論*p*值有多小，這種差異都不是很有趣。你肯定不想基於如此微小的差異來宣告男孩教育危機吧？正是因為這個原因，越來越多的人開始使用某種效果量的標準量度，與假設檢定的結果一起報告。假設檢定本身告訴你，你應該相信你所觀察到的效應是否是真實的（即不僅僅是偶然的），而效果量告訴你是否應該關心這種效應。

[^09-hypothesis-testing-10]: 注意，這裡的「p」與*p*值無關。在 jamovi 的二項檢定中，$p$ 參數對應到根據虛無假設所作出的正確反應的機率。換句話說，它是 $\theta$ 的值。
...
不出所料，科學家們十分關注如何最大化他們實驗的考驗力。我們希望實驗能夠成功，因此希望在虛無假設錯誤時能夠最大程度地拒絕它（當然，我們通常希望虛無假設是錯誤的！）。正如我們所見，影響考驗力的因素之一是效果量。因此，增加考驗力的第一件事就是增加效果量。在實踐中，這意味著您要設計您的研究，使效果量得到放大。例如，在我的ESP研究中，我可能相信超感官力量在安靜、昏暗的房間裡，且干擾要少一些，才能得到最佳效果。因此，我會嘗試在這樣的環境中進行實驗。如果我可以某種方式增強人們的ESP能力，那麼$\theta$的真實值就會增加[^09-hypothesis-testing-11]，因此我的效果量將會更大。簡而言之，巧妙的實驗設計是提高考驗力的一種方式，因為它可以改變效果量。

[^09-hypothesis-testing-11]: 需要注意的是，真實的母體參數$\theta$不一定對應於自然界中不可改變的事實。在這種情況下，$\theta$只是人們正確猜測房間裡卡片顏色的概率。因此，母體參數可以受到各種因素的影響。當然，前提是超感官力量實際存在！

不幸的是，即使擁有最佳的實驗設計，你可能只得到一個小效應。例如，也許超感官知覺確實存在，但即使在最佳情況下，它也非常微弱。在這種情況下，增加樣本量是增加考驗力的最佳方法。一般來說，你擁有的觀察數量越多，就越有可能區分兩種假設。如果我用10個參與者進行了超感官知覺的實驗，其中7個人正確猜出了隱藏卡片的顏色，你可能不會特別印象深刻。但如果我用10,000個參與者進行實驗，其中7,000個人猜對了，你就更有可能認為我發現了什麼。換句話說，樣本量增加，考驗力也增加。在@fig-fig9-8中展示了這一點，該圖顯示了對於真實參數$\theta = 0.7$，對於從1到100的所有樣本量$N$的檢定功效，其中假設虛無假設預測$\theta_0 = 0.5$。
...
因為考驗力很重要，當你在考慮進行一個實驗時，知道你可能擁有多少考驗力會非常有用。當然，你無法確定真正的效果大小，但有時可以猜測它應該有多大。如果是這樣，你就可以猜測你需要多大的樣本量！這個想法叫做**考驗力分析**，如果可以做到，它非常有幫助。它可以告訴你關於是否有足夠的時間或金錢來成功地進行實驗的一些信息。現在越來越多的人認為，考驗力分析應該成為實驗設計的必要部分，因此了解它很重要。然而，我在本書中沒有討論考驗力分析。這部分原因有點無聊，也有點實質性的。無聊的原因是我沒有時間去寫關於考驗力分析的內容。實質性的原因是我對考驗力分析還有些懷疑。作為一位研究者，我很少有機會進行考驗力分析。這可能是因為 (a) 我的實驗有些不標準，不知道如何正確定義效果量，或者 (b) 我對效果量實在是一無所知，不知道如何解釋分析結果。此外，我曾與一位擔任統計咨詢工作的人進行過廣泛交流（碰巧是我妻子），實際上她唯一有人請求考驗力分析的情況，是當她在幫助某人撰寫申請補助金的申請書時。換句話說，在現實生活中，科學家們似乎只有在被強制進行這種分析時才想要進行考驗力分析，而且這並不是他們日常工作的一部分。簡而言之，我認為，儘管考驗力是一個重要的概念，但考驗力分析並不像人們所說的那麼有用，除非在少數情況下，（a）有人已經找到了如何計算您實際實驗設計的考驗力的方法，（b）您對效果量的可能大小有很好的想法。也許其他人的經歷比我更好[^09-hypothesis-testing-12]，但我個人從未遇到過(a)和(b)都成立的情況。也許未來我會改變看法，未來版本的這本書會包括更詳細的考驗力分析討論，但目前來說，這是我對這個主題感到舒適的說法。有一個可能的例外情況是，當研究人員研究新的醫學治療的有效性時，他們預先指定要檢測的重要效果量，例如超過現有治療的任何效果。通過這種方式，可以獲得有關新治療的潛在價值的一些信息。

[^09-hypothesis-testing-12]: 有一個可能的例外是，當研究人員研究一種新的醫療治療方法的有效性時，他們事先指定了需要檢測的重要效果量，例如在任何現有治療之上的額外效果量。這樣可以獲得一些有關新治療方法潛在價值的信息。
...
虛無假設檢定是統計理論最普遍的一種應用。大部份科學研究報告都會在結果呈現某種假設的檢定。現代科學研究者幾乎都要多少了解*p*值的意義，否則不能被認為是合格的科學研究者 ，所以這一章是本書最重要的部分。以下幫助讀者及學生快速回顧本章重點：

- [假設大觀園](09-Hypothesis-testing.html#%E5%81%87%E8%A8%AD%E5%A4%A7%E8%A7%80%E5%9C%92) 研究假設與統計假設。虛無假設與對立假設。
- [兩種決策失誤](09-Hypothesis-testing.html#%E5%85%A9%E7%A8%AE%E6%B1%BA%E7%AD%96%E5%A4%B1%E8%AA%A4) 型一與型二錯誤
...
- 統計推論的[決策要素](09-Hypothesis-testing.html#%E7%B5%B1%E8%A8%88%E6%8E%A8%E8%AB%96%E7%9A%84%E6%B1%BA%E7%AD%96%E8%A6%81%E7%B4%A0)
...
- [假設檢定實作須知](09-Hypothesis-testing.html#%E5%81%87%E8%A8%AD%E6%AA%A2%E5%AE%9A%E5%AF%A6%E4%BD%9C%E9%A0%88%E7%9F%A5) 
- [效果量、樣本量、考驗力](09-Hypothesis-testing.html#sec-Effect-size-sample-size-and-power)
- 一些[值得繼續學習的主題](09-Hypothesis-testing.html#%E5%80%BC%E5%BE%97%E7%B9%BC%E7%BA%8C%E5%AD%B8%E7%BF%92%E7%9A%84%E4%B8%BB%E9%A1%8C)

到了本書最後一章@sec-Bayesian-statistics ，我會從貝氏統計觀點回顧統計理論與虛無假設檢定，還有介紹幾套非典型取向的統計工具。不過到此為止，我們暫別抽象的統計理論，接下來將幾章將學習實用的統計分析方法。

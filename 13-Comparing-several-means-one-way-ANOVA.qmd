...
前面的步驟說明牽涉很多抽象符號，而且只讀字面不能充分理解，在此我們做個總結。回到一開始的[獨立樣本變異數分析示範資料]，根據這份資料的描述統計表，我們知道各種藥劑的情緒改善平均分數：安慰劑是 $0.45$，Anxifree 是 $0.72$，Joyzepam 是 $1.48$。有了這些資訊和計算公式，回到1899年就能用鉛筆和紙辦個統計桌遊[^13-comparing-several-means-one-way-anova-7]。不過因為現在不是1899年，人人手上都有計算機，讓我們偷個懶，只要做前5筆資料計算就好。我們先從計算組內離均差平方和$SS_w$ 開始，要畫張像 @tbl-tab12-3 的表格來輔助計算。


[^13-comparing-several-means-one-way-anova-7]: 更真實的描述應該是 "1899年的統計學家著迷於計算的主要原因，是因為沒有什麼朋友，也沒有比算術更有趣的事情可做。因為直到 1920 年，ANOVA 並未來到人類世界。"
...
我們終於完成了，值得慶祝對吧！有了檢定統計值，最後就是判斷這個統計值是否代表結果是顯著的。如同在 @sec-Hypothesis-testing 討論過的手作方法，有心的讀者可以翻開任何一本統計教科書最後的附錄，查找其中有關F檢定的表，只要找到指定 $\alpha$值所對應的閾值，例如 $0.05$，$0.01$ 或 $0.001$，搭配 自由度2 和15。設定$\alpha$為 $0.001$ 的話，會查到F的閾值是$11.34$。因為小於最後算出的F值，可以宣稱$p < 0.001$。但這是時代的眼淚，今天各式各樣統計軟體都能為我們算出精確的p值。這個範例的p值是 $0.000071$。除非我們對型一錯誤率採取非常謹慎的立場，我們幾乎 可以結論這樣的研究結果能拒絕虛無假設。
...
演練到這裡，讀者應該不大想純靠手工整理這樣的報表，幾乎所有專業統計軟體，包括 jamovi，都能將ANOVA 的結果統整成像 @tbl-tab12-8 的表格，所以大家最好習慣一下。儘管軟體能輸出完整的 ANOVA 報表，真正的報告並不需要置入整份表格，現代學術寫作規範所建議的報告格式是類似以下的句型：

> *單因子變異數分析顯示，測試的藥物對情緒改善有顯著影響，F(2,15) = 19.3，p < .001。*

天哪！我們做了這麼多事，只為了寫出這短短一行。
...
若是讀者認真地一邊閱讀前一節的示範步驟，一邊在計算紙上演算，肯定能了解變異數分析程序有多麼複雜，而且一不小心就會出錯。接下來還有許多計算，如果還是用紙筆演練，各位會覺得幹麼要浪費時間做這些事？
...
為了讓大家從複雜的變異數分析計算中解脫，就要靠jamovi了！請開啟`ANOVA`模組的ANOVA設定選單，將`mood.gain`搬到"Dependent Variable"對話框，再將`drug`搬到"Fixed Factors"對話框，就能在報表區看到如同 @fig-fig12-3 的結果。[^13-comparing-several-means-one-way-anova-9] 請留意設定視窗列出的選項，還要勾選 $\eta^2$ ，報表也有顯示這項計算結果，稍後討論效果量會進一步說明。
...
[^13-comparing-several-means-one-way-anova-9]: 與用手計算的數值相比，jamovi 的結果更為準確，這是因為四捨五入的差異。

jamovi 的結果表格包含平方和、自由度以及目前還沒有真正談到的其他指標。不過，請各位留意，jamovi的報表給的標籤名稱並不是「組間」和「組內」，反而使用符合內容的名詞做為標籤。在這一節的範例裡，組間離均差平方和是對應藥物對結果變項的影響，組內離均差平方和是對應“殘餘”的變異，因此大家習慣稱“殘差”(Residual)。比較jamovi計算的數值與 @sec-How-ANOVA-works-example 手動計算的數值，會看到兩者或多或少是相同的，除了四捨五入的差異。組間平方和為 $SS_b = 3.45$，組內平方和為 $SS_w = 1.39$，各自的自由度為 $2$ 和 $15$。還有報告 F 值和 p 值，也是和手動計算的數字差不多，只是四捨五入的差異。
...
前面有認真演練計算過程的同學一看就懂。 @fig-fig12-3 展示的ANOVA 報表有呈現$\eta^2$，簡單驗算一下，因為$SS_b = 3.45$ 還有 $SS_tot = 3.45 + 1.39 = 4.84$，所以可以算出：

$$\eta^2=\frac{3.45}{4.84}=0.71$$

解讀$\eta^2$同樣直接了當，這項效果量公式就是指結果變項的變異，可以被預測變項解釋的比例。$\eta^2=0$ 代表兩種變項之間完全無關，$\eta^2=1$則表示兩者關係密切。值得一提的是，$\eta^2$與 @sec-The-R2-value 提過的是同一種效果量家族成員，有效力相等的解釋力。許多統計教科書建議使用 $\eta^2$ 作為 ANOVA的預設效果量指標，不過荷蘭心理學者Daniel Lakens在一篇有趣的<a href="https://daniellakens.blogspot.com/2015/06/why-you-should-use-omega-squared.html" target="_blank">blog文章</a>提到，以統計實務處理的真實資料來看，$\eta^2$可能不是最好的效果量指標，因為這是一個有偏誤的估計值。還好jamovi還可以選擇使用另一個估計偏誤較少的指標 $\omega^2$ (omega squared) 。
...
將虛無假設分解為三個宣告後，就能將八種可能的分析結果表列，如同 @tbl-tab12-9  的內容。
...
既然已經拒絕虛無假設，第一種分析結果已經出局。接著就要問，其餘七種可能的分析結果，那一個才是*正確的*結論？面對這樣的局面，最後再看一次描述統計報告，像是 @fig-fig12-1 的統計圖，我們可以看到 Joyzepam 比安慰劑和 Anxifree好，但是Anxifree 和安慰劑的效果沒有差別是最有可能的結論。不過，若是要提出精確的報告，就需要做進一步的檢定。
...
我們要如何進行多重檢定？這裡要比較三對分組平均值：安慰劑 vs. Anxifree，安慰劑 vs. Joyzepam，和 Anxifree vs. Joyzepam，最簡單的方法是執行三次獨立樣本t檢定，逐一確認檢定結果。用 jamovi 很容易做到。打開 ANOVA 設定視窗的 'Post Hoc Tests'(事後檢驗)選項，將 'drug'(藥物)變項移到右側的活動框中，然後勾選 'No correction'(無校正)核取方塊。就會產生一張整齊的表格，顯示藥物變項的三對比較 t 檢定結果，如同 @fig-fig12-4 的展示。
...
#| label: fig-fig12-4
#| classes: .enlarge-image
#| fig-cap: 成對 t 檢定的事後比較~並列未經校正及Holm校正的p值。
...
前面提到一次做多組t檢定可能會有問題，最大的問題是沒有明確的理論指引時，同時執行多組t檢定，就只是看一看有那幾組會出現顯著結果，這樣一來科學分析很容易就會變成一場「魷魚遊戲」。這種對多組差異進行缺乏理論基礎的比較，稱為**事後分析**（"post hoc" 是拉丁語，意為 "在真正要緊的事務之後"）。[^13-comparing-several-means-one-way-anova-10]

[^13-comparing-several-means-one-way-anova-10]:若是研究開始前已經有些理論，預期某幾組比較是*確實*有意義的，那就是另一回事了。這種情況所執行多重檢定，其實不是「事後分析」，而是「預先計劃的比較」。下一個單元 @sec-The-method-of-planned-comparisons 有更進一步介紹。

事後分析不是不可以做，而是要非常小心。例如前面介紹的[成對t檢定]應該儘量不要做，因為每組t檢定的型一錯誤率都設定為.05的話，執行了三組t檢定，會導致型一錯誤率膨脹。想像有件資料要用ANOVA分析10組平均值，然後挑出其中45筆組間差異進行「事後」t檢定，找出那些組別之間有顯著差異。如同 @sec-Hypothesis-testing 討論過的，那幾組會有顯著差異純粹是偶然的。虛無假設檢定有效的基本原則是，分析者嚴格控制型一錯誤率，為了確定造成ANOVA顯著結果的原因，不管三七二十一執行多組t檢定，己經導致全體45組的型一錯誤率膨脹到天邊了。

通常的解決方法是校正p值，控制總體型一錯誤率[@Shaffer1995]。這種事後分析進行的校正，通常稱為**多重比較校正**，有時也叫「即時推論」(simultaneous inference)。其實統計學界已經開發出許多校正方法，在這一節還有下一個單元 @sec-Post-hoc-tests 將討論其中幾種，本書介紹的只是最常見的校正方法[有與趣的讀者請參考 @Hsu1996 ]。
...
Bonferroni校正是其中最簡單的一種方法[@Dunn1961]。如果現在要處理m個獨立的計檢定，這個方法能確保即使出現*任何*顯著結果，維持總型一錯誤機率上限在$\alpha$。[^13-comparing-several-means-one-way-anova-11] 如同字面的定義，Bonferroni校正只是「將所有原始 p 值乘以 m」。做法是設定校正前的p值為$p$，校正後的p值為$p_j^{'}$，根據Bonferroni校正，校正前後的p值關係是：

[^13-comparing-several-means-one-way-anova-11]: 順帶一提，並不是所有校正方法都是這樣搞，因為Bonferroni校正是著眼於控制「全族型一錯誤率」(family wise Type I error rate)。有些校正方法則是從控制「偽發現率」(False Discovery Rate)下手。


$$p_j^{'}=m \times p$$

採用Bonferroni校正，只要確認$p_j^{'} < \alpha$，就能正確拒絕虛無假設。這種方法的邏輯非常簡單：現在要進行m組獨立的統計檢定，如果設定每個檢定的型一錯誤率上限是$\frac{\alpha}{m}$，所有檢定的*總*型一錯誤率就不會大於 $\alpha$。這種方法簡單到Bonferroni在他的論文裡寫道：

>因為這個方法如此簡單，又容易使用，我肯定以前一定有人用過。但是我翻遍任何文獻都找不到，只能推測：正是因為這種方法簡單到讓每個自認聰明絕頂的統計學家，不覺得這種方法在一些研究狀況的好處[@Dunn1961，第52-53頁]。

要讓 jamovi 執行Bonferroni校正，只需要點選「校正」選項中的「Bonferroni」核選項目， ANOVA 結果報表將會出現另一列以Bonferroni校正調整的 p 值（ @tbl-tab12-8 ）。如果我們比較一下這三個 p 值與未校正的成對 t 檢驗的 p 值，很明顯 jamovi 只是將它們乘以 $3$而已。
...
雖然Bonferroni校正非常簡單，但並不是最好的校正方法。另一種常用的方法是**Holm校正**[@Holm1979]。這種方法背後的思路是假設按照p 值的大小，從最小的比較開始，調整到第j個比較的p值為止，以下列公式校正p值

$$p_j^{'}=j \times p_j$$

也就是最大的 p 值保持不變，第二大的 p 值增倍，第三大的 p 值增加三倍，依此類推。或者用以下等式挑出較*大*者。

$$p_j^{'}=p_{j+1}^{'}$$

這樣解釋不大好懂，讓我慢慢說明Holm校正的原理。首先將$m$組檢定的p值按照數值大小排序，從最小的排到最大的。校正最小的 p 值只要乘以 $m$，其他組的p值校正則需要兩個步驟。像是第二小的 p 值首先要乘以 $m - 1$，若是這個校正後的p值大於前一次校正的p值( $(m-1)\times p_{m-1} > m \times p_m$ )，那就可以停止校正；若是小於前一次校正的p值，就要將原來的p值，改換為前一次校正後的p值。 @tbl-tab12-10 展示五組多重比較檢定的p值，如何用Holm校正來調整每組檢定的p值。
...
雖然校正步驟稍微麻煩，Holm校正有一些優點，像是保證比Bonferroni校正有更低的型二錯誤率，可以提高統計考驗力。此外，Holm校正保障每組檢定的型一錯誤率一致，雖然這套方法執行下來看起來不大像。要跑統計的研究者，搞清楚兩種校正方法的原理後，多數人會支持Holm校正。所以對於還在起步的讀者，Holm校正應該是*首選*的校正方法。 @fig-fig12-4 展示Holm校正的 p 值，最大的 p 值（ Anxifree vs. 安慰劑）和未校正的p值一樣，都是.15。最小的 p 值（Joyzepam vs. 安慰劑）則校正為原來的三倍。
...
儘管理解Kruskal Wallis檢定法是讓許多人望而生畏的任務，實際操作卻相當輕鬆因為jamovi的ANOVA模組有包括名為“Non-parametic”(無母數)-"One-way ANOVA(Kruskall-Wallis)"的分析選單。大多資料是像*clinicaltrial.csv*一樣，其中有結果變項`mood.gain`和分組變項`drug`。如此就能直接用jamovi執行Kruskall-Wallis檢定，結果如同 @fig-fig12-8 展示，寫成報告格式$\chi^2 =12.076, df = 2, p = 0.00239$。
...
#| label: fig-fig12-8
#| classes: .enlarge-image
#| fig-cap: 無母數單因子ANOVA/Kruskall-Wallis分析結果
...
要用jamovi執行單因子重覆量數ANOVA，只要點選ANOVA - Repeated Measures ANOVA，就能打開如 @fig-fig12-9 的分析設定視窗。
...
jamovi報表區接著輸出如 @fig-fig12-10 至 @fig-fig12-13 的單因子重覆量數變數分析報表。建議各位先查看有沒有符合Mauchly球形檢定( @fig-fig12-10 )，這是檢核各條件之間的變異是否相等(也就是研究條件之間的得分差異值分佈大致相同)。 @fig-fig12-10 顯示Mauchly檢定的$p = .720$，根據一般的顯著水準（p > .05），我們有理由相信各測量條件之間的差異值分佈無顯著差異，符合球性分佈的適用條件。
...
這兩個校正的F值都可以透過“適用條件檢核”選單的球形修正選項指定，修正過的F值將顯示在變異數分析結果表，如同 @fig-fig12-11 顯示。
...
從這次範例資料的分析，我們發現Mauchly的球形檢定並不顯著(p = .720，即p > 0.05)。因此，這表示我們可以接受已滿足資料差異的分佈符合球形假設，不需要校正F值進行。報告可以用原始F值說明"任務"的主要效果之統計資訊：$F = 6.93$，$df = 2$，$p = .013$，所以三種任務正確率的差異，確實是因為各種任務的測試目標所造成的（$F(2, 10) = 6.93$，$p = .013$）。

與獨立樣本變異數分析一樣，jamovi也可以執行重覆量數變數分析的事後檢定。示範結果如同 @fig-fig12-12 。這顯示言語任務與語任務的正確率有顯著差異，但是其他各項任務之間的正確率沒有顯著差異。
...
運用描述統計可以幫助解釋結果，只要設定邊際均值選項，生成如 @fig-fig12-13 的表格。比較不同任務的平均正確次數，可以看出布洛卡失語症患者的言語生成任務（平均= 7.17）和概念理解任務（平均= 6.17）表現相對較好。不過，句子修正任務的表現明顯較差（平均= 4.33），事後檢定顯示言語生成和句子修正的正確率有明顯差異。
...
和Kruskall-Wallis檢定相同，徹底解釋的話需要先知道複雜的數學知識，所以這裡不會詳細介紹。對於本書的目標讀者來說，只要了解jamovi輸出的Friedman檢定報表是已經校正的結果，  @fig-fig12-14 展示的報表是用前一節的失語症患者資料做Friedman檢定的範例。
...
#| label: fig-fig12-14
#| classes: .enlarge-image
#| fig-cap: “無母數重覆量數變數分析”設定視窗與結果報表
#The 'Repeated Measures ANOVA (Non-parametric)' dialogue box and results in jamovi
knitr::include_graphics("images/fig13-14.png")
```

要用jamovi執行Friedman檢定非常簡單。只要從功能模組選單，選擇"ANOVA" - Repeated Measures ANOVA (Non-parametric)，就能開啟如 @fig-fig12-14 的選單。然後將要比較的重複測量資料變項名稱（言語、概念、語法）放到“測量”(Measures)對話框。想要生成描述統計表格，只要勾選描述(Descriptives)。

jamovi報表區顯示描述統計、卡方統計值、自由度和p值。由於p值小於通常用於一般的顯著水準（p < .05），我們可以下結論：布洛卡失語症患者在言語生成（中位數= 7.5）和概念理解（中位數= 6.5）的表現較好。然而，他們的句子修正任務表現明顯較差（中位數= 4.5），事後檢定顯示言語生成和句子修正任務的表現有顯著差異。

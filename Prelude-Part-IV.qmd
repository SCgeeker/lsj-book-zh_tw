...
本書的第IV部分是迄今為止最理論的部分,因為它專注于統計推論理論。在接下來的三章中,我的目標是為您介紹概率論、抽樣和在 @sec-Estimating-unknown-quantities-from-a-sample 中估計參數,以及在 @sec-Hypothesis-testing 中統計假設檢定。在我們開始之前,我想談談全局性內容。統計推論主要是關于從數據中學習。其目標不再僅僅是描述我們的數據,而是利用數據得出有關世界的結論。為了激發討論,我想花些時間談論一個被稱為歸納謎題的哲學難題,因為它談到了一個在整本書中一再出現的問題:統計推論依賴假設。這聽起來不妙。在日常生活中,人們常說“永遠不要做任何假設”之類的話,並且心理課程經常談論假設和偏見是我們應該避免的壞事情。根據痛苦的個人經歷,我已經學會永遠不在哲學家面前說這樣的話!  
...
> _整個戰爭藝術在於瞭解山的另一邊是什麼,或者換句話說,從我們所知道的東西中學習我們不知道的東西。_

關於這句名言,據說是在一次鄉村馬車旅行中產生的。威靈頓公爵和他的同伴 J. W. Croker 玩着一個猜謎遊戲,各自試圖預測每座山的另一邊會是什麼。結果證明,在每一案例中威靈頓都是對的,而 Croker 都是錯的。多年后當有人詢問威靈頓這場遊戲時,他解釋說“整個戰爭的藝術在於瞭解山的另一邊是什麼”。的確,戰爭在這方面並不特殊。生活的所有方面都是某種形式的猜謎遊戲,而想要度過一天又一天,我們需要做出好的猜測。那麼,讓我們玩一場自己的猜謎遊戲。[^translation-1]
...
歸納作為哲學難題最常與大衛·休謨(David Hume)和納爾遜·古德曼(Nelson Goodman)的哲學工作聯繫在一起,但你可以在各個領域找到這個問題的例子,如文學作家(像是寫愛麗絲夢遊境的[路易斯·卡羅](https://zh.wikipedia.org/wiki/%E8%B7%AF%E6%98%93%E6%96%AF%C2%B7%E5%8D%A1%E7%BE%85) )和機器學習([“免費午餐”定理](https://blog.maxkit.com.tw/2019/07/nflno-free-lunch-theorems.html))。嘗試“從我們所知道的中學習我們不知道的東西”確實有些怪異。關鍵點是,如果你想了解世界上的任何事物,則預設條件[^Preloude_translation_1]和偏見是不可避免的。這是無法逃避的,統計推論和人類推理一樣也是如此。在對話中,我試圖採取合理的人類推理,但是你所依賴的常識推理跟統計學家所做的没有區別。你在對話中“常識”那一半所依賴的隱含預設條件是 Wellesley 和 Croker 之間在技能上存在某些區別,你要做的就是設法找出他們之間的技能差異大小。而我的“邏輯分析”完全拒絕這種預設條件。我所願意接受的只是存在勝利和失敗的序列,并且我不知道會觀察到哪些序列。在整個對話中,我堅持認為在 Wellesley-Croker 比賽開始時所有在邏輯上可能的數據集都同樣合理,我修改信念的唯一方式就是消除與觀察事實不一致的可能性。


[^Preloude_translation_1]: 譯註~同學們也許在一些地方看過"hypothesis"與"assumption"都被翻成「假設」，但是在英文這兩個詞是有區別的。"hypothesis"是指根據某種科學理論，設定實驗結果可能是什麼樣子。"assumption"是指研究運用的收集與分析資料的技術，必須符合什麼條件才會有效。因此"hypothesis"翻成「假設」是合理的，"assumption"應該翻成「預設條件」。本書的用詞將依此原則翻譯。
